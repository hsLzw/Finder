...
from FINDERmaster.code.FINDER_CN import PrepareBatchGraph
import graph
from FINDERmaster.code.FINDER_CN import nstep_replay_mem
from FINDERmaster.code.FINDER_CN import nstep_replay_mem_prioritized
from FINDERmaster.code.FINDER_CN import mvc_env
from FINDERmaster.code.FINDER_CN import utils
...
#################################################New code for FINDER#####################################
    ...
    def fit(self,g_list,covered,actions,list_target):
        ...
        for i in range(0,n_graphs,BATCH_SIZE):
            ...
            loss += result[0]*bsize
        return loss / len(g_list)
    def EvaluateRealDataForApi(self, model_file, points, edges, stepRatio=0.0025):  
        cdef double solution_time = 0.0
        g = nx.Graph()
        new_edge = []
        for edge in edges:
            new_edge.append(
                (points.index(edge[0]), points.index(edge[1]))
            )

        g.add_nodes_from(range(len(points)))
        g.add_edges_from(new_edge)
        print("==========evalueate real data begin==========")
        if stepRatio > 0:
    ...
    def Train(self):
        ...
        save_dir = 'FINDERmaster/code/FINDER_CN/models/Model_powerlaw'
